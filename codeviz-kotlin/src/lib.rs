use codeviz_core::{LanguageParser, CodeGraph, GraphMeta, Node, NodeKind, Edge, EdgeKind, ParseError};
...
impl KotlinParser {
    ...
    fn traverse<'a>(
        ...
        match kind {
            ...
            "function_declaration" => self.handle_function_declaration(node, source, graph, file_path),
            "companion_object" => {
    ...
    fn handle_import<'a>(&self, node: TSNode<'a>, source: &str, graph: &mut CodeGraph, file_path: &str) {
        let mut cursor = node.walk();
    ...
    fn handle_class_declaration<'a>(&self, node: TSNode<'a>, source: &str, graph: &mut CodeGraph, file_path: &str) {
        let mut cursor = node.walk();
        ...
        let label = if is_data { format!("{}[data]", class_name) } else { class_name.clone() };
        let kind = if is_interface { NodeKind::Interface } else { NodeKind::Class };
                    ...
                    if inner_child.kind() == "constructor_invocation" {
                        ...
                        for tc in inner_child.children(&mut tc_cursor) {
                            if tc.kind() == "user_type" {
                                let target = tc.utf8_text(source.as_bytes()).unwrap_or("").to_string();
                                graph.edges.push(Edge {
                        ...
                        let target = inner_child.utf8_text(source.as_bytes()).unwrap_or("").to_string();
                        graph.edges.push(Edge {
                            from_id: class_name.clone(),
                            to_id: target,
                            kind: if is_interface { EdgeKind::Inherits } else { EdgeKind::Implements },
                        });
                    }
                }
            }
    ...
    fn handle_object_declaration<'a>(&self, node: TSNode<'a>, source: &str, graph: &mut CodeGraph, file_path: &str) {
        let mut cursor = node.walk();
    ...
    fn handle_function_declaration<'a>(&self, node: TSNode<'a>, source: &str, graph: &mut CodeGraph, file_path: &str) {
        let mut cursor = node.walk();
...
"#;
        ...
        let functions: Vec<_> = graph.nodes.iter().filter(|n| matches!(n.kind, NodeKind::Function { .. })).collect();
        assert_eq!(functions.len(), 2);

        let fetch_fn = functions.iter().find(|n| n.id == "fetch").unwrap();
        assert!(matches!(fetch_fn.kind, NodeKind::Function { is_async: true }));

        let run_fn = functions.iter().find(|n| n.id == "run").unwrap();
        assert!(matches!(run_fn.kind, NodeKind::Function { is_async: false }));
    }
}
