...
# We want to refactor lines 654-866.
# We will define a `pub struct RunResult { pub parsed_count: usize, pub nodes: usize, pub edges: usize }`
# And `pub fn run_core(run_args: &RunArgs, is_check: bool, quiet: bool) -> Result<(bool, RunResult, Vec<codeviz_core::parser::ParseError>), String>`
...
pub fn run_core(
    ...
    if is_check {
        let config = match &run_args.config_path {
            Some(path) => {
                codeviz_core::Config::load_from_file(std::path::Path::new(path)).unwrap_or_default()
            }
    ...
    match run_args.format {
        ...
        OutputFormat::Mermaid => {
            ...
            for target in outputs {
                ...
                if is_check {
                    let prev_quiet = quiet; // Do we want check_diagram_up_to_date to print? It currently uses println. We will just pass it, maybe it prints if not quiet. Wait, check_diagram_up_to_date always prints. We can let it print.
            ...
            if is_check {
                return Ok((
                    ...
                    RunResult {
                        parsed_count,
                        nodes: merged_graph.meta.node_count,
                        edges: merged_graph.meta.edge_count,
                    },
                    parse_errors,
                ));
            }
...
replacement = """    if args.len() > 1 && (args[1] == "run" || args[1] == "check") {
...
# Find the block in main.rs
start_idx = content.find('    if args.len() > 1 && (args[1] == "run" || args[1] == "check") {')
end_idx = content.find('    // Default error for unknown subcommands', start_idx)
